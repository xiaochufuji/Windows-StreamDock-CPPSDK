...
unsigned char* StreamDock293V3::getFirmVersion(int lenth) {
    return transport->getInputReport(lenth);
}


int StreamDock293V3::setBrightness(int percent) {

    return this->transport->setBrightness(percent);
}

int StreamDock293V3::setBackgroundImg(std::string path)
{
    // 读取图像文件
    ...
    // 保存旋转后的图像
    cv::imwrite("293v3bg347274857239.jpg", Image2);
    int res = this->transport->setBackgroundImgDualDevice("293v3bg347274857239.jpg");
    std::remove("293v3bg347274857239.jpg");
    return res;
}


int StreamDock293V3::setBackgroundImgData(unsigned char* imagedata) {
    ...
    try
    {
        ...
        //使用flip函数实现180°旋转 -1表示沿x和y轴翻转
        cv::flip(img, img, -1);
        cv::imwrite("293v3BGData347274857239.jpg", img);
        int res = this->transport->setBackgroundImgDualDevice("293v3BGData347274857239.jpg");
        // 删除临时文件
        std::remove("293v3BGData347274857239.jpg");
        return res;
    }
...
void StreamDock293V3::read(std::vector<unsigned char>& vec)
{
    vec.clear();
...
unsigned char* StreamDock293V3::read()
{
    unsigned char* command = new unsigned char(13);
    if (this->transport->read(command, 13) != -1)
        return command;
...
int StreamDock293V3::setKeyImgData(unsigned char* imagedata, int key)
{
    try
    {
        key = transform(key);
